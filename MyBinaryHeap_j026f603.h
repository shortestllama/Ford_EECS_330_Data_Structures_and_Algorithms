...
class MyBinaryHeap 
{
  private:
    ...
    void percolateUp(const size_t p) 
    {
        ...
        ComparableType copy = x;
        data[0] = std::move(copy);

        for (; x > data[temp / 2]; temp /= 2) {
            data[temp] = std::move(data[temp / 2]);
        }

        data[temp] = std::move(data[0]);
        // code ends
    }
  ...
  public: 
	...
	void enqueue(const ComparableType& x)
    {
        ...
        for (; x > data[p / 2]; p /= 2) {
            data[p] = std::move(data[p / 2]);
        }

	    data[p] = std::move(data[0]);
        // code ends
    } 
